...
if (bot) {
  ...
  bot.onText(/^\/(start|menu)$/i, async (msg) => {
    ...
    await sendAndTrack(chatId, bot.sendMessage, [
      'Откройте приложение → перейдите в «Профиль» → затем откройте «Главная» и выберите одно из четырёх намерений → включите режим «Лайв» → нажмите на круг. Ваш голос будет засчитан через 60 сек. Подробности — в разделе «О проекте». Для повторного запуска основного меню используйте /start Вопросы и общение — Группа в Телеграмме: t.me/synchroni',
